...
public class RAGConfig extends AbstractTaskPropertiesConfig {
    ...
    public boolean shouldEmbed(iped.data.IItem item) {
        ...
        // Block synthetic individual sub-items whose consolidated HTML report is already embedded
     ...
     *   1. MIME blacklist veto (absolute): binary MIME types are always blocked, even
     *      if the item's category matches the whitelist. This prevents binary files
     *      (e.g. raw registry hives, executables) from slipping through when a broad
     *      parent category (e.g. "Windows Registry") is whitelisted.
     *   2. MIME whitelist fast-path: surgical types (csv, audio, etc.) are always allowed.
     *   3. Category whitelist: O(1) pre-computed HashSet.contains() — zero allocations,
     *      scales to billions of items without GC pressure.
     *
     * Blacklist mode logic:
     *   1. MIME blacklist veto (same absolute veto as whitelist mode).
